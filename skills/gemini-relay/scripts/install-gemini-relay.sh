#!/usr/bin/env bash
# Install the Gemini relay binary launcher. Hooks are opt-in (--with-hooks) —
# the relay's session-file tail is the default integration and covers tool
# events + thinking + final reply without requiring Gemini CLI hooks.
...
Options:
  ...
  --with-hooks             Also install Gemini CLI hooks (scuttlebot-check/post/after-agent)
...
if [ "$WITH_HOOKS" = "1" ]; then
  ...
  printf '  hooks:      (skipped — pass --with-hooks to install CLI hooks)\n'
fi
printf '  launcher:   %s\n' "$LAUNCHER_DST"
printf '  env:        %s\n' "$CONFIG_FILE"
