		...
		if err != nil {
			...
			if err := conn.Connect(connectCtx); err != nil {
				...
				if err := relay.JoinChannel(ctx, sessionChannel); err != nil {
					fmt.Fprintf(os.Stderr, "codex-relay: session channel provision: %v\n", err)
				} else {
					cfg.Channels = mergeChannels(cfg.Channels, []string{sessionChannel})
				}

				filtered = buildFilteredConnector(relay, cfg)
				// Apply daemon-pushed channel resolutions on top of env defaults.
				// Best-effort: an older daemon (no /v1/relay/config) returns 404.
...
func filterMessagesState(messages []message, since time.Time, nick, agentType string, state *relayState, handoffBudget int) ([]message, time.Time) {
	...
	for _, msg := range ordered {
		...
		if isActivityPost && state != nil && !state.consumeHandoff(msg.Channel, handoffBudget) {
			debugf("codex-relay: filter drop agent→agent from=%s: handoff budget exhausted on %s\n", msg.Nick, msg.Channel)
			continue
		}
...
func loadConfig(args []string) (config, error) {
	...
	if nick == "" {
		nick = defaultRelayNick("codex", target, cfg.SessionID)
	}
	...
	reconcileIRCDefaults(&cfg, fileConfig, "codex-relay")
	return cfg, nil
}
...
// with a one-line WARN — see issue #161.
